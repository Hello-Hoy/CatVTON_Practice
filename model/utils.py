...
def init_adapter(
    ...
    **kwargs,
    ...
    for name in unet.attn_processors.keys():
        current_cross_attention_dim = None if name.endswith("attn1.processor") else cross_attn_dim
        ...
        if current_cross_attention_dim is None:
            if self_attn_cls is not None:
                attn_procs[name] = self_attn_cls(
                    hidden_size=hidden_size,
                    cross_attention_dim=current_cross_attention_dim,
                    **kwargs,
                )
            else:
...
def init_diffusion_model(diffusion_model_name_or_path, unet_class=None):
    ...
    try:
        ...
        unet.load_state_dict(
            torch.load(os.path.join(unet_folder, "diffusion_pytorch_model.bin"), map_location="cpu"),
            strict=True,
        )
    except Exception:
        unet = None
